...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public boolean addFollowee(String name1, String name2) {
        User pointer1 = this.getUser(name1);
    ...
    public String recommendWhoToFollow(String name) {
        ...
        User pointer = getUser(name);
        String nameToFollow = "";
        for (int i = 0; i < userCount; i++){
            if (users[i].getName().equals(name)){
                continue;
            }
            amountOfFollowers = pointer.countMutual(users[i]);
            if (amountOfFollowers > max){
                max = amountOfFollowers;
                nameToFollow = users[i].getName();
            }
        }
        return nameToFollow;
    }
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++){
            ...
            if (numberOfOccurences > max){
                ...
                nameOfMostPopular = users[i].getName(); 
            }
        }
        return nameOfMostPopular;
    }
    ...
    private int followeeCount(String name) {
        int count = 0;
        for (int i = 0; i < userCount; i++){
            if (users[i].getName().equals(count)){
                continue;
            }
